...
class BLSClassifier:
    ...
    def decision_function(self, X: np.ndarray) -> np.ndarray:
        """
        输出 BLS 的原始类别响应 / logits: O = H @ beta，形状为 (N, C)。

        注意：这里不做 Softmax。DER-BLMoE 的论文公式要求先得到原始专家输出 O(i)，
        再统一映射为概率 p(i)，因此 MoE 门控应调用本函数，避免二次 Softmax。
        """
    ...
    # 兼容常见命名：后续如果其他文件想显式获取 raw output，也可以调用 predict_logits。
    predict_logits = decision_function

    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        """
        预测类别概率 (N, C)。

        对单个 BLS 分类器而言，本函数仍然返回 Softmax 概率，保证 baseline BLS
        和其他调用 predict_proba 的代码不受影响。
        """
