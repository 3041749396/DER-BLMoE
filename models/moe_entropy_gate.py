...
class MoEBLSEntropyResidualGate:
    ...
    修改说明：
    1. [Math Fix] 专家先输出原始响应 O=H@beta，再只做一次 Softmax，避免二次 Softmax。
    ...
    def __init__(
            ...
            # 门控参数
            gate_alpha: float = 2.0,  # 控制对熵残差的敏感度
    ...
    def predict_proba_with_details(self, X_bands: List[np.ndarray]):
        """
        返回最终融合概率以及专家级诊断信息。

        诊断信息用于 exp_test.py 中的可解释性分析：
        - expert_probs: 每个专家的类别概率，形状 (N, M, C)
        - expert_preds: 每个专家的独立预测，形状 (N, M)
        - entropies: 每个专家输出分布的 Shannon 熵，形状 (N, M)
        - raw_scores: DER Top-k 稀疏化后的未归一化得分，形状 (N, M)
        - final_weights: DER 最终归一化权重，形状 (N, M)
        - topk_mask: 专家是否被 Top-k 选中的 0/1 掩码，形状 (N, M)
        """
        if len(self.experts) == 0:
            raise RuntimeError("模型尚未训练或未正确加载专家，请先调用 fit() 或加载已训练模型。")

        probs_list = []
        logits_list = []

        # 1. 获取所有专家的原始输出 O(i)，再统一做一次 Softmax。
        for exp, xb in zip(self.experts, X_bands):
            xb_float = xb.astype(np.float64)
            raw_output = exp.decision_function(xb_float)
            p = stable_softmax(raw_output, axis=1)
            logits_list.append(raw_output)
            probs_list.append(p)

        logits_stack = np.stack(logits_list, axis=1)  # (N, Experts, Classes)
        ...
        # 3. DER 动态熵残差得分 + Top-k 稀疏化。
        raw_scores = self._compute_gate_weights(entropies)  # 已经过 Top-k mask
        topk_mask = (raw_scores > 0).astype(np.int32)

        # 4. 归一化权重。
        sum_weights = raw_scores.sum(axis=1, keepdims=True) + 1e-12
        final_weights = raw_scores / sum_weights
        ...
        details = {
            "expert_logits": logits_stack,
            "expert_probs": p_stack,
            "expert_preds": p_stack.argmax(axis=2),
            "entropies": entropies,
            "entropy_norm": entropies / self.max_entropy,
            "entropy_residual": self.last_entropy_residual,
            "raw_scores": raw_scores,
            "final_weights": final_weights,
            "topk_mask": topk_mask,
        }
        return final_probs, details

    def predict_proba(self, X_bands: List[np.ndarray]) -> np.ndarray:
        final_probs, _ = self.predict_proba_with_details(X_bands)
        return final_probs
