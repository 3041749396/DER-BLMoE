...
class PyTorchTrainer:
    """
    通用 1D 深度网络训练器：
    - 支持主流程使用的 ResNet18_1D
    - 输入假定为实部+虚部拼接的一维向量 (N, 2*L)，内部自动 reshape 为 (N, 2, L)
    - 始终跑满 epochs，使用验证集选取最佳 epoch 的参数，不做早停。

    配置策略：
    - 如果 batch_size / epochs / lr / random_state 未显式指定，则从 MoeBLSConfig 中读取：
        DL_BATCH_SIZE, DL_EPOCHS, DL_LR, GLOBAL_SEED
    ...
    def __init__(
        ...
        if random_state is None:
            random_state = getattr(cfg, "GLOBAL_SEED", 2025)
    ...
    def _build_model(self) -> nn.Module:
        ...
        if self.model_name == "resnet18":
            ...
            raise ValueError(
                ...
                "the main pipeline only supports 'resnet18'."
            )
    ...
    def fit(
        ...
        train_generator.manual_seed(self.random_state)
