...
from models.ajcn_official_adapter import OfficialAJCNTrainer
...
def run_training_pipeline(
    ...
    # AJCN official baseline.
    # This uses the official AJCN BaseModel/PrunedModel/DSC/PPO modules preserved under third_party/AJCN-main,
    # with only a raw-IQ -> 32x32x3 data adapter for this project.
    logger.info("  [Train] AJCN official strict integration (RAW -> 32x32x3).")
    ajcn_trainer = OfficialAJCNTrainer(cfg=cfg, device=dl_device, seed=seed, logger=logger)
    t0 = time.time()
    ajcn_result = ajcn_trainer.fit(X_raw_tr, Y_tr, X_raw_val, Y_val)
    t_ajcn = time.time() - t0
    training_times["AJCN"] = t_ajcn
    logger.info(f"    -> AJCN Training+Compression Time: {t_ajcn:.4f}s")
    save_model_torch(ajcn_result.model, os.path.join(run_dir, "run_ajcn_best.pth"), logger)
    save_model_pickle(ajcn_result.info, os.path.join(run_dir, "run_ajcn_info.pkl"), logger)
...
def main():
    ...
    logger.info("\n[STFT Energy-Cropping Summary]")
    logger.info("\n" + build_stft_crop_report(cfg))
