...
from models.ajcn_official_adapter import build_official_ajcn_model_from_info, raw_iq_to_ajcn_tensor
...
def load_models_and_pca(run_path, cfg, device, input_len, logger):
    ...
    model_files = [
        ...
        ("AJCN", "run_ajcn_best.pth", "pytorch_ajcn"),
    ]
...
def visualize_snr_academic(df, output_root, logger):
    ...
    markers = {"DER-BLMoE (Ours)": "o", "SFEBLN": "^", "BLS": "s", "ResNet18": "v", "AJCN": "D"}
    linestyles = {"DER-BLMoE (Ours)": "-", "SFEBLN": "-.", "BLS": "--", "ResNet18": ":", "AJCN": ":"}
...
def run_snr_experiment():
    ...
    crop_indices = calculate_global_crop_indices(tr_raw, cfg, energy_ratio=energy_ratio)
        ...
        for snr in snr_levels:
            ...
            for m_name, (model, m_type) in models.items():
                if m_name == "BLS":
                    ...
                    X_in = inputs["raw"]
                    if m_type == "pytorch_ajcn":
                        X_torch = torch.tensor(raw_iq_to_ajcn_tensor(X_in), dtype=torch.float32).to(device)
                    else:
                        L = X_in.shape[1] // 2
